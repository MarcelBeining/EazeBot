...
class tradeHandler:
    ...
    def __reduce__(self):
        ...
        return (
        self.__class__, (self.exchange.__class__.__name__, None, None, None, None, self.message), self.__getstate__(),
        None, None)
                ...
                if 'actualAmount' not in trade:
                    ...
                    if fee['currency'] == ts['coinCurrency']:
                        trade['actualAmount'] = trade['amount'] - (
                            fee['cost'] if self.exchange.name.lower() != 'binance' or self.getFreeBalance(
                                'BNB') < 0.5 else 0)  # this is a hack, as fees on binance are deduced from BNB if this is activated and there is enough BNB, however so far no API chance to see if this is the case. Here I assume that 0.5 BNB are enough to pay the fee for the trade and thus the fee is not subtracted from the traded coin
                    else:
                        trade['actualAmount'] = trade['amount']
    ...
    def checkNum(self, *value):
        return all(
            [(isinstance(val, float) | isinstance(val, int)) if not isinstance(val, list) else self.checkNum(*val) for
             val in value])
            ...
            try:
                ...
                if count >= 5:
                    if iTs:
                        self.unlockTradeSet(iTs)
                    if printError:
                        self.message('Order not found error 5 times in a row on %s%s' % (self.exchange.name,
                                                                                         '' if iTs is None else ' for tradeSet %d (%s)' % (
                                                                                         list(
                                                                                             self.tradeSets.keys()).index(
                                                                                             iTs), self.tradeSets[iTs][
                                                                                             'symbol'])), 'Error')
                    raise (e)
                else:
                    time.sleep(0.5)
    ...
    def activateTradeSet(self, iTs, verbose=True):
        ...
        if self.sumSellAmounts(iTs, 'notinitiated') - (self.sumBuyAmounts(iTs, 'notfilled') + ts['coinsAvail']) > 0:
            self.message(
                'Cannot activate trade set because the total amount you (still) want to sell (%s %s) exceeds the total amount you want to buy (%s %s after fee subtraction) and the amount you already have in this trade set (%s %s). Please adjust the trade set!' % (
                self.amount2Prec(ts['symbol'], self.sumSellAmounts(iTs, 'notinitiated', 1)), ts['coinCurrency'],
                self.amount2Prec(ts['symbol'], self.sumBuyAmounts(iTs, 'notfilled', 1)), ts['coinCurrency'],
                self.amount2Prec(ts['symbol'], ts['coinsAvail']), ts['coinCurrency']), 'error')
            return wasactive
        elif self.minBuyPrice(iTs, order='notfilled') is not None and ts['SL'] is not None and ts[
            'SL'] >= self.minBuyPrice(iTs, order='notfilled'):
            self.message(
                'Cannot activate trade set because the current stop loss price is higher than the lowest non-filled buy order price, which means this buy order could never be reached. Please adjust the trade set!',
                'error')
            return wasactive
        ...
        if verbose and not wasactive:
            ...
            self.cost2Prec(ts['symbol'], self.sumSellCosts(iTs) - totalBuyCost), ts['baseCurrency']))
            if ts['SL'] is not None or ts['dailycloseSL'] is not None:
                ...
                loss = totalBuyCost - ts['costOut'] - (
                            ts['initCoins'] + self.sumBuyAmounts(iTs) - self.sumSellAmounts(iTs, 'filled')) * sl
    ...
    def newTradeSet(self, symbol, buyLevels=[], buyAmounts=[], sellLevels=[], sellAmounts=[], sl=None, candleAbove=[],
                    initCoins=0, initPrice=None, force=False):
        self.updateDownState(True)
    ...
    def getTradeSetInfo(self, iTs, showProfitIn=None):
        ...
        'T' if ts['active'] else 'INACTIVE t', list(self.tradeSets.keys()).index(iTs), self.exchange.name,
        ...
        if sumBuys > 0:
            ...
            self.amount2Prec(ts['symbol'], sumBuys), ts['coinCurrency'], self.cost2Prec(ts['symbol'], sum(
                [val[0] * val[1] / sumBuys if sumBuys > 0 else None for val in filledBuys])))
        if sumSells > 0:
            ...
            self.amount2Prec(ts['symbol'], sumSells), ts['coinCurrency'], self.cost2Prec(ts['symbol'], sum(
                [val[0] * val[1] / sumSells if sumSells > 0 else None for val in filledSells])))
    ...
    def createTradeHistoryEntry(self, iTs):
        ...
        if self.numBuyLevels(iTs, 'filled') > 0 or self.numSellLevels(iTs, 'filled') > 0:
            ...
            if gainBTC:
                ...
                gainUSD = None
            self.tradeSetHistory.append({'time': time.time(), 'days': None if not 'createdAt' in ts else (time.time() -
                                                                                                          ts[
                                                                                                              'createdAt']) / 60 / 60 / 24,
    ...
    def addInitCoins(self, iTs, initCoins=0, initPrice=None):
        if self.checkNum(initCoins, initPrice) or (initPrice is None and self.checkNum(initCoins)):
            ...
            if self.getFreeBalance(ts['coinCurrency']) < initCoins:
                ...
                self.amount2Prec(ts['symbol'], self.getFreeBalance(ts['coinCurrency'])), ts['coinCurrency']), 'error')
                return 0
            self.lockTradeSet(iTs)
    ...
    def getTradeParam(self, iTs, what, method, direction, order='all', subtractFee=True):
        ...
        if what == 'amount':
            if order == 'all':
                return func(
                    [(val['amount'] if direction == 'sell' or subtractFee == False else val['actualAmount']) for val in
    ...
    def addBuyLevel(self, iTs, buyPrice, buyAmount, candleAbove=None):
        self.updateDownState(True)
        ts = self.tradeSets[iTs]
        if self.checkNum(buyPrice, buyAmount, candleAbove) or (
                candleAbove is None and self.checkNum(buyPrice, buyAmount)):
            ...
            if not self.checkQuantity(ts['symbol'], 'amount', buyAmount):
                ...
                return 0
            elif self.getFreeBalance(ts['baseCurrency']) < buyAmount * buyPrice + fee['cost'] if fee['currency'] == ts[
                'baseCurrency'] else 0:
                self.message('Adding buy level failed, your balance of %s does not suffice to buy this amount%s!' % (
                ts['baseCurrency'],
                ' and pay the trading fee (%s %s)' % (self.fee2Prec(ts['symbol'], fee['cost']), ts['baseCurrency']) if
                fee['currency'] == ts['baseCurrency'] else ''), 'error')
                return 0

            if fee['currency'] == ts['coinCurrency']:
                boughtAmount = buyAmount - (fee['cost'] if (
                            self.exchange.name.lower() != 'binance' or self.getFreeBalance(
                        'BNB') < 0.5) else 0)  # this is a hack, as fees on binance are deduced from BNB if this is activated and there is enough BNB, however so far no API chance to see if this is the case. Here I assume that 0.5 BNB are enough to pay the fee for the trade and thus the fee is not subtracted from the traded coin
            else:
                boughtAmount = buyAmount
            self.lockTradeSet(iTs)
            wasactive = self.deactivateTradeSet(iTs)
            ts['InTrades'].append({'oid': None, 'price': buyPrice, 'amount': buyAmount, 'actualAmount': boughtAmount,
                                   'candleAbove': candleAbove})
            if wasactive:
                self.activateTradeSet(iTs, False)
            self.unlockTradeSet(iTs)
        ...
        if self.checkNum(iTrade, price, amount):
            ...
            if ts['InTrades'][iTrade]['oid'] == 'filled':
                ...
                if not self.checkQuantity(ts['symbol'], 'amount', amount):
                    ...
                    return 0
                elif self.getFreeBalance(ts['baseCurrency']) + ts['InTrades'][iTrade]['amount'] * \
                     ts['InTrades'][iTrade]['price'] < amount * price + fee['cost'] if fee['currency'] == ts[
                    'baseCurrency'] else 0:
                    self.message(
                    ...
                    return 0
                if fee['currency'] == ts['coinCurrency']:
                    boughtAmount = amount - (fee['cost'] if (
                                self.exchange.name.lower() != 'binance' or self.getFreeBalance(
                            'BNB') < 0.5) else 0)  # this is a hack, as fees on binance are deduced from BNB if this is activated and there is enough BNB, however so far no API chance to see if this is the case. Here I assume that 0.5 BNB are enough to pay the fee for the trade and thus the fee is not subtracted from the traded coin
                else:
                    boughtAmount = amount
                ...
                if ts['InTrades'][iTrade]['oid'] is not None and ts['InTrades'][iTrade]['oid'] != 'filled':
                    ...
                    if returnVal == 0.5 and self.getFreeBalance(ts['baseCurrency']) + ts['InTrades'][iTrade]['amount'] * \
                       ts['InTrades'][iTrade]['price'] < amount * price + fee['cost'] if fee['currency'] == ts[
                        'baseCurrency'] else 0:
                        self.message(
                            'Changing buy level failed, your balance of %s does not suffice to buy this amount%s!' % (
    ...
    def setSLBreakEven(self, iTs):
        ...
        if ts['initCoins'] > 0 and ts['initPrice'] is None:
            self.message(
                'Break even SL cannot be set as you this trade set contains %s that you obtained beforehand and no buy price information was given.' %
                ts['coinCurrency'])
            return 0
        elif ts['costOut'] - ts['costIn'] > 0:
            self.message(
                'Break even SL cannot be set as your sold coins of this trade already outweigh your buy expenses (congrats!)! You might choose to sell everything immediately if this is what you want.')
            return 0
        elif ts['costOut'] - ts['costIn'] == 0:
    ...
    def cancelOrder(self, oid, iTs, typ):
        ...
        try:
            return self.safeRun(lambda: self.exchange.cancelOrder(oid, symbol), False)
        except OrderNotFound as e:
            self.unlockTradeSet(iTs)
            raise (e)
    ...
    def fetchOrder(self, oid, iTs, typ):
        symbol = self.tradeSets[iTs]['symbol']
        try:
            return self.safeRun(lambda: self.exchange.fetchOrder(oid, symbol), False)
        except OrderNotFound as e:
            self.unlockTradeSet(iTs)
            raise (e)
        except ccxt.ExchangeError as e:
        ...
        try:
            if specialCheck != 2:
                if self.exchange.has['fetchTickers']:
                    ...
                    func = lambda sym, iTs: tickers[sym] if sym in tickers else self.safeRun(
                ...
                try:
                    ...
                    if not specialCheck:
                        ...
                        for iTrade, trade in enumerate(ts['InTrades']):
                            if 'time' in trade and (datetime.datetime.now() - trade['time']).days > 358 and (
                                ...
                                self.message(
                                    'Time since buy level #%d of trade set %d (%s) on exchange %s was filled approaches one year (%s) after which gains/losses are not eligible for reporting in the tax report in most countries!' % (
                                    iTrade, indTs, ts['symbol'], self.exchange.name,
                                    (trade['time'] + datetime.timedelta(days=365)).strftime('%Y-%m-%d %H:%M')),
                                    'warning')
                        continue
                    orderExecuted = 0
                        ...
                        if trade['oid'] == 'filled':
                            ...
                            try:
                                ...
                                if orderInfo['status'].lower() in ['closed', 'filled']:
                                    ...
                                    self.price2Prec(ts['symbol'], orderInfo['price']), ts['symbol'], self.exchange.name,
                                    self.amount2Prec(ts['symbol'], orderInfo['amount']), ts['coinCurrency'],
                                    self.cost2Prec(ts['symbol'], orderInfo['cost']), ts['baseCurrency']))
                                    ...
                                    if orderInfo['cost'] > 0:
                                        ...
                                        self.message(
                                            'Buy order (level %d of trade set %d on %s) was canceled by exchange or someone else (reason: %s) but already partly filled! Treating order as closed now and updating trade set info.' % (
                                            iTrade, list(self.tradeSets.keys()).index(iTs), self.exchange.name, reason),
                                            'error')
                                    else:
                                        ts['InTrades'][iTrade]['oid'] = None
                                        self.message(
                                            'Buy order (level %d of trade set %d on %s) was canceled by exchange or someone else (reason:%s)! Will be reinitialized during next update.' % (
                                            iTrade, list(self.tradeSets.keys()).index(iTs), self.exchange.name, reason),
                                            'error')
                                ...
                                self.lockTradeSet(iTs)
                                self.message(
                                    'Buy order id %s for trade set %s not found on %s! Maybe exchange API has changed? Resetting order to "not initiated", will be initiated on next trade set update!' % (
                                    str(ts['InTrades'][iTrade]['oid']), ts['symbol'], self.exchange.name), 'error')
                                ts['InTrades'][iTrade]['oid'] = None
                    ...
                    if not specialCheck:
                        ...
                        for iTrade, _ in enumerate(ts['OutTrades']):
                            if ts['OutTrades'][iTrade]['oid'] is None and ts['coinsAvail'] >= ts['OutTrades'][iTrade][
                                'amount']:
                                try:
                                    response = self.safeRun(lambda: self.exchange.createLimitSellOrder(ts['symbol'],
                                                                                                       ts['OutTrades'][
                                                                                                           iTrade][
                                                                                                           'amount'],
                                                                                                       ts['OutTrades'][
                                    ...
                                    self.message('Insufficient funds on exchange %s for trade set #%d. \
                                                 Trade set is deactivated now and not updated anymore (open orders are still open)! \
                                                 Free the missing funds and reactivate. \n %s.'
                                                 % (self.exchange.name, list(self.tradeSets.keys()).index(iTs), str(e)),
                                                 'error')
                                ...
                                try:
                                    ...
                                    if any([orderInfo['status'].lower() == val for val in ['closed', 'filled']]):
                                        ...
                                        if orderInfo['cost'] > 0:
                                            ...
                                            self.message(
                                                'Sell order (level %d of trade set %d on %s) was canceled by exchange or someone else (reason: %s) but already partly filled! Treating order as closed now and updating trade set info.' % (
                                                iTrade, list(self.tradeSets.keys()).index(iTs), self.exchange.name,
                                                reason))
                                        else:
                                            ts['OutTrades'][iTrade]['oid'] = None
                                            ts['coinsAvail'] += ts['OutTrades'][iTrade]['amount']
                                            self.message(
                                                'Sell order (level %d of trade set %d on %s) was canceled by exchange or someone else (reason:%s)! Will be reinitialized during next update.' % (
                                                iTrade, list(self.tradeSets.keys()).index(iTs), self.exchange.name,
                                                reason))
                                    ...
                                    self.lockTradeSet(iTs)
                                    self.message(
                                        'Sell order id %s for trade set %s not found on %s! Maybe exchange API has changed? Resetting order to "not initiated"! If InsufficientFunds errors pop up, the order had probably been executed. In this case please delete the trade set.' % (
                                        str(ts['OutTrades'][iTrade]['oid']), ts['symbol'], self.exchange.name), 'error')
                                    ts['coinsAvail'] += ts['OutTrades'][iTrade]['amount']
                                    ts['OutTrades'][iTrade]['oid'] = None
                        ...
                        if ((ts['SL'] is None and orderExecuted > 0) or (orderExecuted == 2 and (
                                self.sumBuyAmounts(iTs, order='filled') + ts['initCoins'] - self.sumSellAmounts(iTs,
                                                                                                                order='filled')) / (
                                                                                 ts['initCoins'] + self.sumBuyAmounts(
                                                                                 iTs,
                                                                                 order='filled')) < 0.01)) and self.numSellLevels(
                                iTs, 'notfilled') == 0 and self.numBuyLevels(iTs, 'notfilled') == 0:
